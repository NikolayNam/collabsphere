...
Для containerized запуска добавлен отдельный compose-layer:

- `deploy/docker-compose.web.yaml`

Этот слой не только поднимает `web`, но и переопределяет `AUTH_BROWSER_REDIRECT_ORIGINS` у `api` под frontend origin, чтобы browser login не упирался в redirect allowlist.
...
- `WEB_AUTH_BROWSER_REDIRECT_ORIGINS` — allowlist origins для backend browser auth при включённом `docker-compose.web.yaml`; по умолчанию `http://api.localhost:8080,http://collabsphere.localhost:3002`
...
```bash
docker compose --env-file deploy/env/.env.postgres.test -f deploy/docker-compose.postgres-test.yaml --profile test up -d postgres-test
