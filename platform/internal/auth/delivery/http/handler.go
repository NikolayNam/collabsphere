...
import (
	...
	accmapper "github.com/NikolayNam/collabsphere/internal/accounts/delivery/http/mapper"
...
func (h *Handler) Me(ctx context.Context, input *struct{}) (*authdto.MeResponse, error) {
	...
	resp.Body.FirstName = acc.FirstName()
	resp.Body.LastName = acc.LastName()
	resp.Body.Status = string(acc.Status())
	_ = accmapper.ToAccountResponse // можно убрать, если не переиспользуешь mapper
	return resp, nil
}
